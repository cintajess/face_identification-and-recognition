...
# Load the registration numbers and corresponding labels
registered_faces = {
    "jecinta": "jecinta"  # Example registration number for "jecinta"
}
...
def compare_faces(captured_face):
    ...
    # Determine the label based on prediction confidence
    if prediction > 0.5:
        label = "jecinta"
...
def capture_and_verify():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            label, confidence = compare_faces(captured_face)
            
            # If the captured face matches any face in the trained model
            if label == "jecinta" and confidence >= 0.5:
                ...
                if registration_number == registered_faces[label]:
                    print("Verification successful: Face recognized as Jecinta with confidence:", confidence)
                else:
